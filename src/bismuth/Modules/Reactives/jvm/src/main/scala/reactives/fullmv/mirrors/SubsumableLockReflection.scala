...
class SubsumableLockReflection(
  ...
  override def tryLock0(hopCount: Int): Future[TryLockResult0] = {
    ...
    proxy.remoteTryLock().map {
      case RemoteLocked(lock) =>
        if lock eq this then { // note: This must not be an == comparison. The received instance may be a new remote reflection of the same original lock, while this instance was concurrently deallocated and cannot have new references added!
          if Host.DEBUG then
            println(
      ...
      proxy.remoteTrySubsume(lockedNewParent).map {
        ...
        case RemoteBlocked(newParent) =>
          if newParent eq this then {
            ...
            if hopCount > 0 then newParent.localAddRefs(hopCount)
          } else {
            if Host.DEBUG then
              println(
  ...
  override def asyncUnlock0(): Unit = {
    if Host.DEBUG then println(s"[${Thread.currentThread().getName}] $this sending unlock request")
    proxy.remoteUnlock()
  }
  override def remoteUnlock(): Unit = {
    if Host.DEBUG then println(s"[${Thread.currentThread().getName}] $this passing through unlock request")
    proxy.remoteUnlock()
  }
  override def remoteTryLock(): Future[RemoteTryLockResult] = {
    if Host.DEBUG then println(s"[${Thread.currentThread().getName}] $this passing through tryLock request")
  ...
  override def toString: String = {
    ...
    s"SubsumableLockReflection($guid on $host, ${if refs <= 0 then "gc'd" else refs.toString + " refs"})"
  }
}
